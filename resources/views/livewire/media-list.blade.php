<div>
    ...
    <div class="bg-surface-container-low rounded-xl overflow-hidden shadow-2xl relative">
        {{-- Loading Overlay --}}
        <div wire:loading.flex class="absolute inset-0 bg-surface-container-low/50 backdrop-blur-[1px] z-10 items-center justify-center">
        ...
        <div class="divide-y divide-outline-variant/10" wire:key="media-list-results">
            @forelse ($entries as $entry)
                ...
                <div class="grid grid-cols-12 items-center px-8 py-6 hover:bg-surface-container-high transition-colors group" wire:key="entry-{{ $entry->id }}">
                    ...
                    <div class="col-span-3 md:col-span-2 flex justify-end gap-2">
                        ...
                        <form action="{{ route('my-list.destroy', $entry) }}" method="POST" class="inline" onsubmit="return confirm('{{ __('Are you sure you want to remove this entry?') }}')">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                ...
                <div class="py-24 text-center flex flex-col items-center gap-4">
                    ...
                    <p class="text-on-surface-variant/60 text-sm">
                        @if ($status || $type)
                            ...
                            <button wire:click="reset(['status', 'type'])" class="text-primary font-bold ml-1 hover:underline">{{ __('Clear filters') }}</button>
                        @else
