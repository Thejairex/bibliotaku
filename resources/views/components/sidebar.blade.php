<aside class="hidden md:flex flex-col h-full py-8 px-4 w-64 fixed left-0 top-0 rounded-r-xl bg-neutral-950/70 backdrop-blur-xl shadow-2xl shadow-black/40 z-50">
    ...
    <div class="mt-auto px-4 space-y-1">
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit"
                class="flex items-center gap-4 w-full py-3 transition-all duration-300 font-headline font-bold text-sm tracking-wide text-on-surface-variant hover:text-error active:scale-95">
